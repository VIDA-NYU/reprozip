...
If you are running ``reprounzip docker``, you can inspect the Docker container using ``docker``, or start a new one based on the image created by `reprounzip`; all images  generated by ReproZip are named with the ``reprounzip_`` prefix.
