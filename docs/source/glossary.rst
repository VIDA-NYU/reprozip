...
..  glossary::
    ...
    run
        A single command traced by ``reprozip trace [--continue]``. Multiple commands can be traced successively before creating the pack; the reproducer will be able to run them separately using ``reprounzip <unpacker> run <directory> <run-nb>``.
