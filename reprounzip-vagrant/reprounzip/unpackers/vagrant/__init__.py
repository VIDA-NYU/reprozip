...
from reprounzip.common import load_config, record_usage_report
...
def select_box(runs):
    ...
    record_usage_report(vagrant_select_box='%s;%s;%s' % (distribution, version,
                                                         architecture))
...
def vagrant_setup_create(args):
    ...
    record_usage_report(use_chroot=use_chroot,
    ...
    if args.base_image and args.base_image[0]:
        record_usage_report(vagrant_explicit_image=True)
    ...
    if use_chroot:
        ...
        if packages:
            record_usage_report(vagrant_install_pkgs=True)
...
def run_interactive(ssh_info, interactive, cmds, request_pty):
    ...
    if interactive and ssh_exe:
        record_usage_report(vagrant_ssh='ssh')
        ...
        record_usage_report(
                vagrant_ssh='interactive' if interactive else 'simple')
