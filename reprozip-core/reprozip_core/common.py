...
class RPZPack(object):
    ...
    def _open_zip(self):
        ...
        if sys.version_info < (3, 7):
            ...
            os.close(fd)
            self._extract_file('DATA.tar.gz', Path(temporary_data))
            self.data = tarfile.open(temporary_data, mode='r:*')
            atexit.register(os.remove, temporary_data)
        else:
    ...
    def _extract_file(self, name, target):
        if self.tar is not None:
            ...
            member.filename = str(target.parts[-1])
